          ...
          <div className="basic-details">
            ...
            <div className="price-calculations">
              ...
              {(() => {
                ...
                const gstRate = parseFloat(productData.gst || 0);
                const cessRate = parseFloat(productData.cess || 0);
                let basePrice = salePrice;
                let gstAmt = 0;
                let cessAmt = 0;

                if (productData.taxType === 'inclusive') {
                  const totalTaxRate = gstRate + cessRate;
                  basePrice = salePrice / (1 + (totalTaxRate / 100));
                  gstAmt = basePrice * (gstRate / 100);
                  cessAmt = basePrice * (cessRate / 100);
                } else {
                  gstAmt = basePrice * (gstRate / 100);
                  cessAmt = basePrice * (cessRate / 100);
                }
                ...
                return (
                  <>
                    <div className="calc-row">
                      ...
                      <span className="amount">₹{basePrice.toFixed(2)}</span>
                    </div>
                    <div className="calc-row">
                      <span>GST ({gstRate}%):</span>
                      <span className="amount">₹{gstAmt.toFixed(2)}</span>
                    </div>
                    {cessRate > 0 && (
                      <div className="calc-row">
                        <span>CESS ({cessRate}%):</span>
                        <span className="amount">₹{cessAmt.toFixed(2)}</span>
                      </div>
                    )}
                    <div className="calc-row total">
                      <span>Final Amount:</span>
                      <span className="amount">₹{(basePrice + gstAmt + cessAmt).toFixed(2)}</span>
                    </div>
                  </>
                );
              })()}
